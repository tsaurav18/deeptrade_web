...
const parseKoreanManNumber = (koreanNumber) => {
  ...
  const onFilterSubmit = () => {
    ...
    //if market cap or trading Price applied 
    else if (mrktCapCheckBoxStatus && tradingPriceCheckBoxStatus && 
          !tradingVolCheckBoxStatus 
    ) {
      if (mrktCapFilValue.length == 2 && tradePriceFilValue.length==2) {
        ...
        const filtered_data = currentTableDataList.filter((v) => {
          return (
            v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
            v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue[1]) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
            v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1])
          );
        });
        ...
        if (mrktCapFilValue.length == 2 && tradePriceSelFilterType === "이하"  ) {
          const filtered_data = currentTableDataList.filter((v) => {
            return v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
            v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1]) && v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue);
          });

          setTableDataList(filtered_data);
        } else if(mrktCapFilValue.length == 2 && tradePriceSelFilterType === "이상") {
          const filtered_data = currentTableDataList.filter((v) => {
            return v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
            v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1]) && v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue);
          });
        ...
        else if(tradePriceFilValue.length == 2 && mrktCapSelFilterType === "이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
            v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue[1]) && v.marketCap <= parseKoreanEokNumber(mrktCapFilValue);
          });

          setTableDataList(filtered_data);
        }
        else if(tradePriceFilValue.length == 2 && mrktCapSelFilterType === "이상"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
            v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue[1]) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
          });
      ...
      //둘다 이상
      else if(tradePriceSelFilterType==="이상" && mrktCapSelFilterType === "이상"){
        const filtered_data = currentTableDataList.filter((v) => {
          return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
        });

        setTableDataList(filtered_data);
      }
      //둘다 이하
      else if(tradePriceSelFilterType==="이하" && mrktCapSelFilterType === "이하"){
        const filtered_data = currentTableDataList.filter((v) => {
          return v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue) && v.marketCap <= parseKoreanEokNumber(mrktCapFilValue);
        });
       ...
       // 거래대금 이하 and market cap 이상
       else if(tradePriceSelFilterType==="이하" && mrktCapSelFilterType === "이상"){
        const filtered_data = currentTableDataList.filter((v) => {
          return v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
        });
        ...
        else if(tradePriceSelFilterType==="이상" && mrktCapSelFilterType === "이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue) && v.marketCap <= parseKoreanEokNumber(mrktCapFilValue);
          });
...
// When Market Cap and Trading vol both applied at same time
    ...
    tradingVolCheckBoxStatus){
      if (mrktCapFilValue.length == 2 && tradeVolFilValue.length==2) {
        ...
        const filtered_data = currentTableDataList.filter((v) => {
          return (
            v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
            v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue[1]) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
            v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1])
          );
        });
        ...
        if (mrktCapFilValue.length == 2 && tradeVolSelFilterType === "이하"  ) {
          const filtered_data = currentTableDataList.filter((v) => {
            return v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
            v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1]) && v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue);
          });

          setTableDataList(filtered_data);
        } else if(mrktCapFilValue.length == 2 && tradeVolSelFilterType === "이상") {
          const filtered_data = currentTableDataList.filter((v) => {
            return v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
            v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1]) && v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue);
          });
        ...
        else if(tradeVolFilValue.length == 2 && mrktCapSelFilterType === "이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
            v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue[1]) && v.marketCap <= parseKoreanEokNumber(mrktCapFilValue);
          });

          setTableDataList(filtered_data);
        }
        else if(tradeVolFilValue.length == 2 && mrktCapSelFilterType === "이상"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
            v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue[1]) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
          });
        ...
        //둘다 이상
        else if(tradeVolSelFilterType==="이상" && mrktCapSelFilterType === "이상"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
          });

          setTableDataList(filtered_data);
        }
        //둘다 이하
        else if(tradeVolSelFilterType==="이하" && mrktCapSelFilterType === "이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue) && v.marketCap <= parseKoreanEokNumber(mrktCapFilValue);
          });
        ...
        // 거래대금 이하 and market cap 이상
        else if(tradeVolSelFilterType==="이하" && mrktCapSelFilterType === "이상"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
          });
          ...
          // 거래대금 이상 and market cap 이하
          else if(tradeVolSelFilterType==="이상" && mrktCapSelFilterType === "이하"){
            const filtered_data = currentTableDataList.filter((v) => {
              return v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue) && v.marketCap <= parseKoreanEokNumber(mrktCapFilValue);
            });
    ...
    tradingVolCheckBoxStatus){
      if (tradePriceFilValue.length == 2 && tradeVolFilValue.length==2) {
        ...
        const filtered_data = currentTableDataList.filter((v) => {
          return (
            v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
            v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue[1]) && v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
            v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue[1])
          );
        });
        ...
        if (tradePriceFilValue.length == 2 && tradeVolSelFilterType === "이하"  ) {
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
            v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue[1]) && v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue);
          });

          setTableDataList(filtered_data);
        } else if(tradePriceFilValue.length == 2 && tradeVolSelFilterType === "이상") {
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
            v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue[1]) && v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue);
          });
        ...
        else if(tradeVolFilValue.length == 2 && tradePriceSelFilterType === "이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
            v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue[1]) && v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue);
          });

          setTableDataList(filtered_data);
        }
        else if(tradeVolFilValue.length == 2 && tradePriceSelFilterType === "이상"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
            v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue[1]) && v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue);
          });
      ...
      //둘다 이상
      else if(tradeVolSelFilterType==="이상" && tradePriceSelFilterType === "이상"){
        const filtered_data = currentTableDataList.filter((v) => {
          return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue) && v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue);
        });

        setTableDataList(filtered_data);
      }
      //둘다 이하
      else if(tradeVolSelFilterType==="이하" && tradePriceSelFilterType === "이하"){
        const filtered_data = currentTableDataList.filter((v) => {
          return v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue) && v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue);
        });
       ...
       // 거래대금 이하 and market cap 이상
       else if(tradeVolSelFilterType==="이하" && tradePriceSelFilterType === "이상"){
        const filtered_data = currentTableDataList.filter((v) => {
          return v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue) && v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue);
        });

        setTableDataList(filtered_data);
      }
        // 거래대금 이상 and market cap 이하
        else if(tradeVolSelFilterType==="이상" && tradePriceSelFilterType === "이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue) && v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue);
          });
    ...
    //When all three conditions applied 
      ...
      tradingVolCheckBoxStatus){
        if (mrktCapFilValue.length == 2 && tradePriceFilValue.length==2 && tradeVolFilValue.length==2) {
          //Interval
          const filtered_data = currentTableDataList.filter((v) => {
            return (
              v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
              v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue[1]) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
              v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1])&& v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
              v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue[1])
            );
          });
          ...
          if (mrktCapFilValue.length == 2 && tradeVolFilValue.length==2  && tradePriceSelFilterType === "이하"  ) {
            const filtered_data = currentTableDataList.filter((v) => {
              return v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
              v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1]) && 
              v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
            ...
            setTableDataList(filtered_data);
          } else if(mrktCapFilValue.length == 2 && tradeVolFilValue.length==2  && tradePriceSelFilterType === "이상") {
            const filtered_data = currentTableDataList.filter((v) => {
              return v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
              v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1]) && 
              v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
          ...
          else if(mrktCapFilValue.length == 2 && tradePriceFilValue.length==2  && tradeVolSelFilterType === "이하"){
            const filtered_data = currentTableDataList.filter((v) => {
              return v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
              v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1]) && 
              v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
          ...
          else if(mrktCapFilValue.length == 2 && tradePriceFilValue.length==2  && tradeVolSelFilterType === "이상"){
            const filtered_data = currentTableDataList.filter((v) => {
              return v.marketCap >= parseKoreanEokNumber(mrktCapFilValue[0]) &&
              v.marketCap <= parseKoreanEokNumber(mrktCapFilValue[1]) && 
              v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
          ...
          else if(tradeVolFilValue.length == 2 && tradePriceFilValue.length==2  && mrktCapSelFilterType === "이하"){
            const filtered_data = currentTableDataList.filter((v) => {
              return v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
              v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue[1]) && 
              v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
           ...
           else if(tradeVolFilValue.length == 2 && tradePriceFilValue.length==2  && mrktCapSelFilterType === "이상"){
            const filtered_data = currentTableDataList.filter((v) => {
              return v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue[0]) &&
              v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue[1]) && 
              v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue[0]) &&
        ...
        //다 이상
        else if(tradePriceSelFilterType==="이상" && mrktCapSelFilterType === "이상" && tradeVolSelFilterType==="이상"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue) && v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
          });
  
          setTableDataList(filtered_data);
        }
        //다 이하
        else if(tradePriceSelFilterType==="이하" && mrktCapSelFilterType === "이하" && tradeVolSelFilterType==="이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue) && v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue) && v.marketCap <= parseKoreanEokNumber(mrktCapFilValue);
          });
         ...
         else if(tradePriceSelFilterType==="이하" && mrktCapSelFilterType === "이하" && tradeVolSelFilterType==="이상"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue) && v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
          });
        ...
        else if(tradePriceSelFilterType==="이하" && mrktCapSelFilterType === "이상" && tradeVolSelFilterType==="이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice <= parseKoreanManNumber(tradePriceFilValue) && v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
          });
        ...
        else if(tradePriceSelFilterType==="이상" && mrktCapSelFilterType === "이상" && tradeVolSelFilterType==="이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue) && v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue) && v.marketCap >= parseKoreanEokNumber(mrktCapFilValue);
          });
        ...
        else if(tradePriceSelFilterType==="이상" && mrktCapSelFilterType === "이하" && tradeVolSelFilterType==="이하"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue) && v.accTradeVolume <= parseKoreanManNumber(tradeVolFilValue) && v.marketCap <= parseKoreanEokNumber(mrktCapFilValue);
          });
        ...
        else if(tradePriceSelFilterType==="이상" && mrktCapSelFilterType === "이하" && tradeVolSelFilterType==="이상"){
          const filtered_data = currentTableDataList.filter((v) => {
            return v.accTradePrice >= parseKoreanManNumber(tradePriceFilValue) && v.accTradeVolume >= parseKoreanManNumber(tradeVolFilValue) && v.marketCap <= parseKoreanEokNumber(mrktCapFilValue);
          });
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <input
                      style={{ width: "20px", marginRight: 5, height: "20px" }}
                      type="checkbox"
                ...
                >
                  ...
                  >
                    <input
                      ...
                      onChange={() =>
                        setTradingVolCheckBoxStatus(!tradingVolCheckBoxStatus)
                      }
                    />
                    {"  "}
                    ...
                    <>
                      {selectedStockId &&
                        ...
                        tableDataList
                          ...
                          {tableDataList.length===0 &&<div style={{padding:30}}> 조건에 맞는 종목이 없습니다. </div>}
                    </>
                  )}
                </Row>
              </>
